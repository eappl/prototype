...
<form name="faq_modify_form" id="faq_modify_form" action="<?php echo $this->sign; ?>&ac=update" method="post">
		<table widtd="99%" align="center" class="table table-bordered table-striped" widtd="99%">

<td>FAQ ID</td>
<td><?php echo $Faq['FaqId']; ?></td>
</tr>

		<input type="hidden" name="FaqId" id="FaqId" class="span4" value="<?php echo $Faq['FaqId']; ?>"/>

<td>名称</td>
<td><input type="text" name="name" id="name" class="span4"   size="50" value="<?php echo $Faq['name']; ?>"/></td>
</tr>
		...
		<tr class="hover">
			...
			<option value = <?php echo $key; ?> <?php if($key==$Faq['FaqTypeId']) { ?>selected<?php } ?>><?php echo $app['name']; ?></option>
			<?php } } ?>
			</select>
</td>
</tr>
		...
		var options = {
			...
			success:function(jsonResponse) {
				...
				if (jsonResponse.errno) {
					...
					var message = '修改FAQ成功';
					divBox.confirmBox({content:message,ok:function(){windowParent.getRightHtml('<?php echo $this->sign; ?>'+ '&FaqTypeId=' + jsonResponse.FaqTypeId);}});

...
订单状态:
<select name = "ExchangeStatus" id = "ExchangeStatus">
	<option value = -1 <?php if(-1==$ExchangeStatus) { ?>selected<?php } ?>> 全部 </option>
	<?php if (is_array($ExchangeStatusArr)) { foreach ($ExchangeStatusArr as $status => $statusname) { ?>
	<option value = <?php echo $status; ?> <?php if($status==$ExchangeStatus) { ?>selected<?php } ?>> <?php echo $statusname; ?> </option>
		<?php } } ?>
</select>
兑换订单类型:
...
<td style="text-align:center"><?php echo $ExchangeInfo['ExchangeStatusName']; ?></td>

...
<form name="partner_add_form" id="partner_add_form" action="<?php echo $this->sign; ?>&ac=insert" method="post">
<table class="table table-bordered table-striped" width="100%">
<tr>
<th><label for="PartnerId">平台</label></th><td>
	<select name="PartnerId" id="PartnerId">
	<option value="partner">请选择</option>
	<?php if (is_array($oPratnerArr)) { foreach ($oPratnerArr as $partner) { ?><option value="<?php echo $partner['PartnerId']; ?>"><?php echo $partner['name']; ?></option><?php } } ?>
	</select>
...
<tr>
	...
	<?php if (is_array($oAppArr)) { foreach ($oAppArr as $product) { ?><option value="<?php echo $product['AppId']; ?>" <?php if($product['AppId']==$AppId) { ?>selected<?php } ?>><?php echo $product['name']; ?></option><?php } } ?>
	</select></td>
</tr>
<tr>

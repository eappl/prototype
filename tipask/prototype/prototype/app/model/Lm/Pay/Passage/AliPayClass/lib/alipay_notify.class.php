...
class AlipayNotify {
	...
	function verifyNotify(){
		if(empty($_REQUEST)) {//判断POST来的数组是否为空
			...
			unset($_REQUEST["ac"],$_REQUEST["ctl"],$_REQUEST["d"],$_REQUEST["m"],$_REQUEST["c"],$_REQUEST["__utma"],$_REQUEST["__utmz"]);
			//生成签名结果
			$mysign = $this->getMysign($_REQUEST);
						echo $mysign ."<br>". $_REQUEST["sign"];
...
//	}
	...
	function getResponse($notify_id) {
        ...
        if ( substr($veryfy_url, 0, 3)=="\xEF\xBB\xBF")

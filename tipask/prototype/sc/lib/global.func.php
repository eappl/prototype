...
//  前台分页
function front_page($num, $perpage, $curpage, $operation) {
	global $setting;
	$multipage = '';
    $mpurl = SITE_URL . $setting['seo_prefix'] . $operation . '/';
	$url = $mpurl;
	if ($num > $perpage) {
	  ...
	  $multipage = ($curpage - $offset > 1 && $pages > $page ? '<a  href="' . $mpurl . '1' . $setting['seo_suffix'] . '" >首页</a>': '') .
                ($curpage > 1 ? '<a href="' . $mpurl . ($curpage - 1) . $setting['seo_suffix'] . '"  class="n">上一页</a>': '');
        for ($i = $from; $i <= $to; $i++) {
        ...
        $multipage .= ( $curpage < $pages ? '<a class="n" href="' . $mpurl . ($curpage + 1) . $setting['seo_suffix'] . '">下一页</a>' : '') .
                ($to < $pages ? '<a href="' . $mpurl . $pages . $setting['seo_suffix'] . '" >最后一页</a>' : '')  .
         '<span>到第<input type="text" class="goto" id="page"
         		 onmouseover="this.focus();this.select();"/>页<button type="button" onclick="goto(\''.$url.'\');">确认</button></span>';
    }
...
function ajax_questionDetailpage($num, $perpage, $curpage, $operation, $type='',$qtype=0,$questin_type,$date) {
	global $setting;
	$multipage = '';
	$mpurl = SITE_URL . $setting['seo_prefix'] . $operation . '/';
	if ($num > $perpage) {
		...
		$multipage = ($curpage - $offset > 1 && $pages > $page ? '<a  href="javascript:questionTypepage(\''.$mpurl.'\',1,'.$type.','.$qtype.',\''.$questin_type.'\',\''.$date.'\');">首页</a>': '') .
		($curpage > 1 ? '<a href="javascript:questionTypepage(\''.$mpurl.'\','.($curpage - 1).','.$type.','.$qtype.',\''.$questin_type.'\',\''.$date.'\');" class="n">上一页</a>': '');
		for ($i = $from; $i <= $to; $i++) {
			$multipage .= $i == $curpage ? '<span class="current">' . $i . '</span>' :
			'<a href="javascript:questionTypepage(\''.$mpurl.'\','.$i.','.$type.','.$qtype.',\''.$questin_type.'\',\''.$date.'\');" >' . $i . '</a>';
		}
		$multipage .= ( $curpage < $pages ? '<a class="n" href="javascript:questionTypepage(\''.$mpurl.'\','.($curpage + 1).','.$type.','.$qtype.',\''.$questin_type.'\',\''.$date.'\');">下一页</a>' : '') .
		($to < $pages ? '<a href="javascript:questionTypepage(\''.$mpurl.'\','.$pages.','.$type.','.$qtype.',\''.$questin_type.'\',\''.$date.'\');">最后一页</a>' : '')  .
		'<span class="page-skip">到第<input type="text" name="questionTypepage" class="goto" onmouseover="this.focus();this.select();"/>页<button type="button" onclick="questionTypepage(\''.$mpurl.'\',$(this).prev().val(),'.$type.','.$qtype.',\''.$questin_type.'\',\''.$date.'\');" >确认</button></span>' ;
	}

	return $multipage;
}
function ajax_page_detailQuestion($text, $perpage, $curpage, $operation, $type='',$qtype=0,$questin_type,$date)
{
	global $setting;
	$multipage = '';
	$mpurl = SITE_URL . $setting['seo_prefix'] . $operation . '/';
	$multipage = '<a class="n" href="javascript:questionTypepage(\''.$mpurl.'\',1,'.$type.','.$qtype.',\''.$questin_type.'\',\''.$date.'\');">'.$text.'</a>';
	return $multipage;
}
//投诉订单分单
...
function get_que_id($q_cid,$id){
	...
	if(is_array($ask_type)){
		if(!empty($ask_type[$q_cid])) {
			...
			setcookie('quickask',serialize($ask_type),time()+604800,"/", ".5173.com");
		} else {
			$quickask[$q_cid] = $id;
			$type = array_merge($ask_type,$quickask);
			setcookie('quickask',serialize($type),time()+604800);
			setcookie('quickask',serialize($type),time()+604800,"/", ".5173.com");
		}
	}else{
		$quickask[$q_cid] = $id;
		setcookie('quickask',serialize($quickask),time()+604800);
		setcookie('quickask',serialize($quickask),time()+604800,"/", ".5173.com");
	}
...
{
	if(preg_match("/^13[0-9]{1}[0-9]{8}$|15[0-9]{9}$|18[0-9]{9}$|14[0-9]{9}$|17[0-9]{9}$/",$mobilephone))
	{
		return true;//验证通过
	}
	else

...
//                img.style.cssText += ';margin:' + (first.margin||0) +"px;" + 'border:' + (first.border||0) +"px solid #000";
            ...
            ci = opt[0];
            if(opt.length == 1){
            	//2013-4-17  fastDFS 解决图片地址中带有域名的问题
            	if(ci.src.indexOf('http://scadmin.5173.com/') != -1){
            		ci.src = ci.src.replace('http://scadmin.5173.com/', '');         		
            	}
            	if(ci.data_ue_src.indexOf('http://scadmin.5173.com/') != -1){
            		ci.data_ue_src = ci.data_ue_src.replace('http://scadmin.5173.com/', '');         		
            	}

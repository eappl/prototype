...
$GLOBALS['fdfs_network_timeout'] = $config->get("fdfs_network_timeout");  //seconds
$GLOBALS['fdfs_tracker_servers'] = $config->get("tracker_servers");
$GLOBALS['fdfs_tracker_server_index'] = 0;


$GLOBALS['fdfs_tracker_server_count'] = count($GLOBALS['fdfs_tracker_servers']);

$GLOBALS['fdfs_http_params'] = $config->get("fdfs_http_params");



function fdfs_get_tracker_server_by_groupname($group_name)
{	
	$fdfs_tracker_servers = $GLOBALS['fdfs_tracker_servers'];
	$fdfs_tracker_server_count = $GLOBALS['fdfs_tracker_server_count'];
	for($i=0; $i<$fdfs_tracker_server_count; $i++)
	{
		if($fdfs_tracker_servers[$i]["group_name"] == $group_name)
		{
			return $i;
		}
	}
...
/**
* generate http anti-steal token
* @param $file_id file id return by FastDFS server
* @param $ts current timestamp
* @param $secret_key the secret key
* @return anti-steal token
*/
function fdfs_get_token($file_id, $ts, $secret_key)
{
	return md5($file_id . $secret_key . $ts);
}

...
class discuz_application extends discuz_base{
		...
		if($this->init_user) {
			...
			if($discuz_uid) {
				$user = getuserbyuid($discuz_uid, 1);
				 ...
				 $this->convertto5173($user['username']);
	...
	public function convertTo5173($username)
	{
		require_once libfile('cache/userstats', 'function');
		build_cache_userstats();
		if(isset($_COOKIE['_5173auth']))
		{
			$url = "http://passport.5173.com/passport/validatecookie?value=".urlencode($_COOKIE['_5173auth']);
			...
			if(!empty($result) && $result['ResultNo'] == 0)
			{
				...
				{
					$ask_front_name = $result['Ticket']['UserName'];
				}
				$ask_front_name = "admin";
				//已登录5173
				if($ask_front_name!="")
				{
					//已登录discuz
					if($username!="")
					{
						...
						else//不是同一人
						{
							...
							$this->setloginstatus("login",$ask_front_name);				
						}				
					}
					else//未登录discuz
					{
		...
		{		
			...
			//$url="http://passport.5173.com?returnUrl=http://".$_SERVER["SERVER_NAME"] . urlencode($_SERVER["REQUEST_URI"]);
			//header("location: $url");
		}		
	}
	public function setloginstatus($act,$ask_front_name="")
	{
		...
		if($act=="login")
		{
			...
			if($user['0']==-1)
			{
				...
				if($uid>0)
				{
					$url = "member.php?mod=logging&action=login&loginsubmit=yes&infloat=yes&lssubmit=yes&fastloginfield=username&handlekey=ls&password=".$ask_front_name."&quickforward=yes&username=".$ask_front_name;
					header("location: $url");
					require_once libfile('cache/userstats', 'function');
					build_cache_userstats();
					//echo "自动注册成功<br>";

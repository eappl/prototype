...
class helpmodel {
    ...
    // 回复时长
    function get_sorceList($start_time,$end_time,$operatorList,$cid=-1,$cid1=-1,$cid2=-1,$cid3=-1,$cid4=-1){
    	...
    	ON q.id = a.qid
    	WHERE q.time <= '$end_time' AND  q.time >= '$start_time'";
    	if($operatorList) $where .= ' AND a.author in '.$operatorList;
    ...
    //获取响应时长数据
    function get_rs_list($start_time,$end_time,$operatorList,$cid=-1,$cid1=-1,$cid2=-1,$cid3=-1,$cid4=-1){
    	$where = "SELECT
	    	COUNT(a.id) AS reply_count,
	    	a.author,
    ...
    //协助数据统计条件获取
    function get_kwhere($start,$end_time,$operatorList,$did=-1,$orderby=-1,$web_did=''){
    	...
    	$where = "SELECT  
				...
				FROM ask_help AS h
				  LEFT JOIN ask_question AS q
				    ON h.qid = q.id WHERE h.display=0 AND h.start <= '$end_time' AND  h.start >= '$start'";
    ...
    function get_klist($where){
    	...
    	foreach($klist as $k=>$v){
    		...
    		$klist[$k]['satify_rate']  = $satify_sum==0?"0%":round($klist[$k]['pj_my']/$satify_sum*100).'%';
    		$klist[$k]['nsatify_rate'] = $satify_sum==0?"0%":round($klist[$k]['pj_bmy']/$satify_sum*100).'%';
    		$klist[$k]['assess_rate']  = $klist[$k]['handle']==0?"0%":round($satify_sum/$klist[$k]['handle']*100).'%';
    	}
    	return $klist;
    }
    //协助相应时长统计条件获取
    function get_swhere($start,$end_time,$operator='',$did=-1,$web_did=''){
    	...
    	$where = "SELECT
				    ...
				    LEFT JOIN ask_question AS q
				    ON h.qid = q.id
    			 WHERE h.display=0 AND h.start <= '$end_time' AND  h.start >= '$start' ";
    	$operator != '' && $where .= " ".(strlen($operator)>0?"AND h.aid in (".$operator.")":" ");
    	$did != -1 && $where .= " AND h.did='$did'";
    	$web_did != '' && $where   .= " AND h.did NOT IN ($web_did) ";
    	$where .= ' GROUP BY h.aid';
	...
	function updateHelp($qid,$helpInfo)
	{
		foreach($helpInfo as $key => $value)
		{
			$txt[$key] = "`".$key."`='".$value."'";
		}
		$sql = "update ".DB_TABLEPRE."help set ".implode($txt,",")." where qid = ".intval($qid);

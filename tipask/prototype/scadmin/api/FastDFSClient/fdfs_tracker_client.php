...
{
	$fdfs_network_timeout = $GLOBALS['fdfs_network_timeout'];
...
{
	$fdfs_tracker_servers = $GLOBALS['fdfs_tracker_servers'];
	$fdfs_tracker_server_index = $GLOBALS['fdfs_tracker_server_index'];
...
function tracker_get_connectionByGroupName($group_name)
{
	$fdfs_tracker_servers = $GLOBALS['fdfs_tracker_servers'];
	$fdfs_tracker_server_index = $GLOBALS['fdfs_tracker_server_index'];
	$fdfs_tracker_server_count = $GLOBALS['fdfs_tracker_server_count'];
	...
	{
		$fdfs_tracker_server_index = fdfs_get_tracker_server_by_groupname($group_name);
    }

	$server = $fdfs_tracker_servers[$fdfs_tracker_server_index];

	if (fdfs_connect_server($server) == 0) //is_resource($server['sock']) ||
	{
//		$fdfs_tracker_server_index++;
//		if ($fdfs_tracker_server_index >= $fdfs_tracker_server_count)
//		{
...
* @return 0 for success, none zero (errno) for fail
*/
function tracker_query_storage_list($tracker_server, &$storage_server, 
		$group_name, $filename)
{
	return tracker_do_query_storage($tracker_server, $storage_server, 
		...
		$group_name, $filename, $multi_server=false)
{
	...
	if (($result=fdfs_recv_response($tracker_server, -1, $in_buff, $in_bytes)) != 0)
	{
		return $result;
	}
...
function tracker_query_storage_store($tracker_server, &$storage_server, $group_name)
{
	...
	if (($result=fdfs_recv_response($tracker_server, TRACKER_QUERY_STORAGE_STORE_BODY_LEN, 
		$in_buff, $in_bytes)) != 0)
	{
		return $result;
	}

 ...
 *
 * $Id: Db.php 366 2012-12-06 03:35:57Z chenxiaodong $
 */
...
class pdodb
{
	...
	protected $fetch_mode = PDO::FETCH_NAMED;
...
//			throw new Base_Db_Exception($this->dsn . ' connect failed');
	...
	public function insert_update($table, array $bind, array $update,$keywords = '')
	{
		...
		$new_bind = array_values(array_merge($t1,$t3));
...
*/
	...
	public function checkTableExist($TableName)
	{
		...
		echo "show tables where Tables_in_$db like '$table'";
		print_r( $table_list );
	...
	public function createLogTable($table,$sub_string)
	{	
		$table_to_process = $table.$sub_string;
		...
		else
		{
			...
			$sql = str_replace('`' . $tmp[1] . '`', 'IF NOT EXISTS ' . $table_to_process, $sql);
